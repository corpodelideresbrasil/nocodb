 ...
 * Agora inclui tratamento de erro para entradas inválidas.
 */
function getNumeroDoMes(nomeDoMes) {
  if (!nomeDoMes || typeof nomeDoMes !== 'string') {
  ...
  const mesNormalizado = nomeDoMes.toLowerCase().substring(0, 3);
  return mapaDosMeses[mesNormalizado] || null;
}
        ...
        agendamentos.forEach(agendamento => {
          ...
          if (partes) {
            ...
            if (mes) {
              ...
              // Se o mês for inválido, registra um aviso para depuração
              console.warn('Entrada de agendamento ignorada por ter um mês inválido: "' + agendamento + '" na linha de ' + participante);
            }
          }
        });
      }
...
*   **Erro: `TypeError: Cannot read properties of null (reading 'clear')`**
    *   **Causa:** Este é o erro mais comum. Significa que o script não conseguiu encontrar uma das abas da sua planilha (provavelmente a `Agenda Final` ou `Respostas`).
...
*   **Agendamentos Faltando? Como Ver os Logs:**
    *   O script agora é robusto e ignora agendamentos que estão escritos em um formato que ele não entende (ex: sem mês).
    *   Para ver quais dados ele ignorou, no editor de scripts, clique em **"Registros de execução"** na barra lateral esquerda. Isso mostrará todos os avisos de dados que foram pulados.

*   **Opção "Principal" não aparece no Gatilho?**
    *   Se, ao configurar o acionador, a opção "Principal" não estiver disponível no menu "Escolha a implantação que será executada", isso significa que uma implantação formal ainda não foi criada.
    *   **Solução:** Siga as instruções no guia complementar `guia_de_implantacao.md` para criar a implantação.
